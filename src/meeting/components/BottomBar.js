...
export function BottomBar({ bottomBarHeight, setIsMeetingLeft }) {
  ...
  const LeaveBTN = () => {
    const { meetingInfo } = useMeetingAppContext();
    const { leave } = useMeeting();
    const handleFinishMeeting = async () => {
      try {
        const res = await finishMeeting({
          ...meetingInfo,
          isActive: false,
        });
        console.log("Meeting Ended", res);
      } catch (error) {
        toast("Error while leaving the meeting", {
